<aside x-data="{
    ...
    <nav class="flex-1 px-3 space-y-1.5 mt-6 overflow-y-auto scrollbar-hide">
        ...
        @foreach ($proxmoxLocs as $loc)
            <div class="space-y-1">
                ...
                <div x-show="openSections.includes('proxmox_{{ $loc['id'] }}')" x-collapse
                    class="ml-6 pl-4 border-l border-white/10 space-y-1 mt-1">

                    <!-- Datacenter Dashboard Link -->
                    <a href="{{ route('proxmox.datacenter', ['location' => $loc['id']]) }}"
                        class="group flex items-center justify-between px-3 py-2 rounded-lg transition-all {{ request()->fullUrlIs(route('proxmox.datacenter', ['location' => $loc['id']])) ? 'bg-white/10 text-white font-bold ring-1 ring-white/20' : 'text-blue-200 hover:bg-white/5 hover:text-white' }}">
                    ...
                    <div class="space-y-1">
                        <button @click="toggleSection('proxmox_{{ $loc['id'] }}_pve')"
                            class="w-full flex items-center justify-between px-3 py-2 rounded-lg text-blue-200 hover:text-white transition-all group text-xs font-medium">
                            <div class="flex items-center space-x-2">
                        ...
                        <div x-show="openSections.includes('proxmox_{{ $loc['id'] }}_pve')" x-collapse
                            class="ml-3 pl-4 border-l border-white/10 space-y-0.5 mt-1">
                            @foreach ($pveItems as $item)
                                <a href="{{ route('proxmox.vm_detail', ['location' => $loc['id'], 'vm_id' => $item['id']]) }}"
                                    class="group flex items-center space-x-2 py-1.5 px-3 text-[11px] rounded-lg transition-all {{ request()->routeIs('proxmox.vm_detail') && request('vm_id') == $item['id'] && request('location') == $loc['id'] ? 'bg-white/10 text-white font-bold ring-1 ring-white/20' : 'text-blue-300/80 hover:text-white hover:bg-white/5' }}">
                                    @php
                                        $vmIcon = 'monitor';
                                        if ($item['type'] == 'lxc') {
                                            $vmIcon = 'container';
        ...
        <div class="pt-6 pb-2 px-4">
            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-blue-300/60">Services</span>
        </div>
        ...
        <div class="space-y-1">
            <button @click="toggleSection('nextcloud')"
                class="w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-blue-100 hover:bg-white/10 hover:text-white transition-all group">
                <div class="flex items-center space-x-3">
                    <i data-lucide="cloud" class="w-5 h-5 text-emerald-400 group-hover:text-emerald-300"></i>
                    <span class="font-semibold text-[13px]">Nextcloud Hub</span>
                </div>
                <i data-lucide="chevron-right" class="w-4 h-4 text-blue-300 transition-transform duration-300"
                    :class="openSections.includes('nextcloud') ? 'rotate-90 text-white' : ''"></i>
            </button>
            <div x-show="openSections.includes('nextcloud')" x-collapse
                class="ml-6 pl-4 border-l border-white/10 space-y-1 mt-1">
                <a href="{{ route('nextcloud.overview') }}"
                    class="block py-2 px-3 text-xs rounded-lg {{ request()->routeIs('nextcloud.overview') ? 'bg-white/10 text-white font-bold ring-1 ring-white/20' : 'text-blue-200 hover:text-white hover:bg-white/5' }}">Overview</a>
                <a href="{{ route('nextcloud.users') }}"
                    class="block py-2 px-3 text-xs rounded-lg {{ request()->routeIs('nextcloud.users') ? 'bg-white/10 text-white font-bold ring-1 ring-white/20' : 'text-blue-200 hover:text-white hover:bg-white/5' }}">Identity</a>
                <a href="{{ route('nextcloud.storage') }}"
                    class="block py-2 px-3 text-xs rounded-lg {{ request()->routeIs('nextcloud.storage') ? 'bg-white/10 text-white font-bold ring-1 ring-white/20' : 'text-blue-200 hover:text-white hover:bg-white/5' }}">Filesystem</a>
            </div>
        </div>
        ...
        <div class="space-y-1">
            <button @click="toggleSection('system')"
                class="w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-blue-100 hover:bg-white/10 hover:text-white transition-all group">
                <div class="flex items-center space-x-3">
                    <i data-lucide="cog" class="w-5 h-5 text-blue-300 group-hover:text-amber-400"></i>
                    <span class="font-semibold text-[13px]">System Tuning</span>
                </div>
                <i data-lucide="chevron-right" class="w-4 h-4 text-blue-300 transition-transform duration-300"
                    :class="openSections.includes('system') ? 'rotate-90 text-white' : ''"></i>
            </button>
            <div x-show="openSections.includes('system')" x-collapse
                class="ml-6 pl-4 border-l border-white/10 space-y-1 mt-1">
                <a href="{{ route('system.alerts') }}"
                    class="block py-2 px-3 text-xs rounded-lg {{ request()->routeIs('system.alerts') ? 'bg-white/10 text-white font-bold ring-1 ring-white/20' : 'text-blue-200 hover:text-white hover:bg-white/5' }}">Global
                    Alerts</a>
                <a href="{{ route('system.logs') }}"
                    class="block py-2 px-3 text-xs rounded-lg {{ request()->routeIs('system.logs') ? 'bg-white/10 text-white font-bold ring-1 ring-white/20' : 'text-blue-200 hover:text-white hover:bg-white/5' }}">Audit
                    Logs</a>
            </div>
        </div>
    </nav>
