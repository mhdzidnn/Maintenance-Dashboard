...
@section('content')
    <div class="p-8 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-black text-white uppercase tracking-tight">{{ $title ?? 'Proxmox VMs' }}</h1>
        ...
        <div class="grid gap-4">
            @forelse($vms as $vm)
                <div
                    class="bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 hover:border-blue-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10">
                    <div class="flex items-start justify-between mb-6">
                        {{-- VM Info --}}
                        <div class="flex items-start space-x-6">
                            {{-- VM Icon --}}
                            <div
                                class="w-16 h-16 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-blue-500/30 flex items-center justify-center flex-shrink-0">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center space-x-3 mb-2">
                                    ...
                                    @if ($vm->is_running)
                                        <span
                                            class="px-3 py-1 bg-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-wider rounded-full border border-emerald-500/30 flex items-center space-x-1.5">
                                            <div class="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
                                            <span>Running</span>
                                        </span>
                                    @else
                                        <span
                                            class="px-3 py-1 bg-red-500/20 text-red-400 text-xs font-bold uppercase tracking-wider rounded-full border border-red-500/30 flex items-center space-x-1.5">
                                ...
                                <div class="flex items-center space-x-4 text-sm text-slate-400">
                                    ...
                                    @if ($vm->formatted_uptime)
                                        <span>
                                            ...
                                            <span
                                                class="font-semibold text-slate-300 ml-1">{{ $vm->formatted_uptime }}</span>
                                        </span>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="flex flex-col space-y-2">
                            <button
                                class="px-4 py-2 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 rounded-lg border border-blue-500/30 transition-all flex items-center space-x-2 text-sm font-semibold">
                    ...
                    <div class="grid grid-cols-3 gap-6">
                        ...
                        <div class="bg-slate-900/50 rounded-xl p-4 border border-slate-700/30">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span
                                    class="text-sm font-bold text-white">{{ number_format($vm->cpu_usage_percent, 1) }}%</span>
                            </div>
                            ...
                            <div class="mt-2 text-xs text-slate-500">
                                {{ $vm->cpu_cores }} vCPU(s) @
                                {{ number_format(($vm->cpu_usage_percent * $vm->cpu_cores) / 100, 2) }} of
                                {{ $vm->cpu_cores }} CPU
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-900/50 rounded-xl p-4 border border-slate-700/30">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span
                                    class="text-sm font-bold text-white">{{ number_format($vm->memory_usage_percent, 1) }}%</span>
                            </div>
                            <div class="w-full bg-slate-700/50 rounded-full h-2.5 overflow-hidden">
        ...
        <div class="grid grid-cols-3 gap-4 mt-6">
            <div
                class="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 backdrop-blur-xl border border-emerald-500/30 rounded-2xl p-6">
                <div class="flex items-center space-x-3 mb-2">
            ...
            <div
                class="bg-gradient-to-br from-red-500/10 to-red-600/5 backdrop-blur-xl border border-red-500/30 rounded-2xl p-6">
                <div class="flex items-center space-x-3 mb-2">
            ...
            <div
                class="bg-gradient-to-br from-blue-500/10 to-purple-600/5 backdrop-blur-xl border border-blue-500/30 rounded-2xl p-6">
                <div class="flex items-center space-x-3 mb-2">
